export { default as audioParams } from './audio-params.json';
export { default as audio } from './audio.json';

export { default as color } from './color.json';
export { default as containerStyle } from './container-style.json';
export { default as fontFamily } from './font-family.json';
export { default as fontSize } from './font-size.json';
export { default as fontStyles } from './font-styles.json';
export { default as font } from './font.json';
export { default as geoPosition } from './geo-position.json';
export { default as gridLayout } from './grid-layout.json';
export { default as iframe } from './iframe.json';
export { default as image } from './image.json';
export { default as images } from './images.json';
export { default as imageStyle } from './image-style.json';
export { default as letterSpacing } from './letter-spacing.json';
export { default as lineHeight } from './line-height.json';
export { default as link } from './link.json';
export { default as map } from './map.json';
export { default as margin } from './margin.json';
export { default as markers } from './markers.json';
export { default as marker } from './marker.json';
export { default as objectFit } from './object-fit.json';
export { default as screenLayout } from './screen-layout.json';
export { default as size } from './size.json';
export { default as slide } from './slide.json';
export { default as slides } from './slides.json';
export { default as textStyle } from './text-style.json';
export { default as url } from './url.json';
export { default as videoControl } from './video-control.json';
export { default as videoControls } from './video-controls.json';
export { default as videoParams } from './video-params.json';
export { default as video } from './video.json';
